...
function mapNames (data, info) {
  return data.map(item => {
    return Object.keys(item).map((i) => {
      ...
      return { key, val }
    }).sort((a, b) => a.key.localeCompare(b.key)).reduce((o, { key, val }) => ({ ...o, [key]: val }), {})
  })
}
...
function rank (series) {
  const ranks = series[0].data.map((_, i) => {
    return series.map((s, i2) => ({ ...s, i: i2 })).sort((a, b) => b.data[i].y - a.data[i].y).reduce((o, s, i3) => ({ ...o, [s.i]: i3 }), {})
  })
  return series.map((s, i) => ({ ...s, data: s.data.map((d, i2) => ({ ...d, _y: d.y, y: ranks[i2][i] + 1 })) }))
}
...
export function pieTransform ({ data, query }, info) {
  ...
  return keys.reduce((a, k) => [...a, { id: k, value: named[k] }], []).sort((a, b) => a.value - b.value)
}
